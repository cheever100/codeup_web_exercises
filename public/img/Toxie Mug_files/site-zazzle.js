...
var s_account="zazzlecom"
var s=s_gi(s_account)
/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. */
s.debugTracking=false
s.trackingServer="track.www.zazzle.com"
s.trackingServerSecure="tracks.www.zazzle.com";
s.visitorNamespace="zazzle"
s.dc=112;
...
s.linkInternalFilters="javascript:,zazzle.com";
s.linkLeaveQueryString=false;
s.linkTrackVars="None";
s.linkTrackEvents="None";
s.navTimingCookie = "s_nt";
s.headerFlyoutCookie = "s_hf";
...
function s_doPlugins(s)
{
	...
	var qsp = get_cookie('qsp',s.d.cookie);

	if (!s.campaign) {
		var cmpn = s.getQueryParam('CMPN');
		if (!cmpn && qsp) {
			
			cmpn = get_qsparam('CMPN',qsp);
		}
		s.campaign=s.getValOnce(cmpn,"s_campaign",0);
	}
	...
	if(s.campaign) {
		...
		s.pageURL += (s.pageURL.indexOf('?')>=0?'&':'?')+'CMPN='+s.campaign;
	}

	
	if(s.getQueryParam('email')) {
		s.evar19=s.getValOnce(s.getQueryParam('email'),"s_evar19",0);
	}

	/* Plugin Example: getCartOpen v1.0 */
	...
	// This code was causing "event1" to be sent as "event11event1"
	...
	*/
	...
	s.getNavTiming = getNavTiming; //have to do this or else SiteCatalyst will treat the function as an object not a function...fml
	s.getNavTiming();

    s.getHeaderFlyout = getHeaderFlyout;
	s.getHeaderFlyout();
}
s.doPlugins=s_doPlugins;

function get_cookie(cookie_name, cookie_string)
{
	var cookie_value = (cookie_name && cookie_string) ? cookie_string.match( '(^|;) ?' + cookie_name + '=([^;]*)(;|$)' ) : '';
	var retval = (cookie_value && cookie_value[2]) ? unescape( cookie_value[2] ) : '';
	return retval;
}

function get_qsparam(param_name, param_string)
{
	var param_value = (param_name && param_string) ? param_string.match( '(^|[\\?&])' + param_name + '=([^&#]*)' ) : '';
	var retval = (param_value && param_value[2]) ? param_value[2] : '';
	return retval;
}

/**** Rachel's custom plugin to get nav timing ***/
function getNavTiming() {
	var
		s = this,
		cookieValues = s.c_r(s.navTimingCookie),
		splitCookieValues = (cookieValues && cookieValues.length) ? cookieValues.split(',') : '',
		t = new Date()
	;

	if (splitCookieValues && splitCookieValues.length) {
		s.prop62 = splitCookieValues[0];
		s.prop63 = splitCookieValues[1];
		s.prop64 = splitCookieValues[2];
		s.prop65 = splitCookieValues[3];
		
		//clear the cookie
		t.setTime(t.getTime() + 1800000);
		s.c_w(s.navTimingCookie, '', t);
	}
}

function getHeaderFlyout() {
	var
		s = this,
		cookieValue = s.c_r(s.headerFlyoutCookie),
		t = new Date()
	;

	if (cookieValue && cookieValue.length) {
		s.event35 = 1;
		s.prop66 = cookieValue;
		
		//clear the cookie
		t.setTime(t.getTime() + 1800000);
		s.c_w(s.headerFlyoutCookie, '', t);
	}
